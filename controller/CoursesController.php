...
function handleEdit()
{
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        if (isset($_POST['btnSave'])) {
            ...
            if (empty($name)) {
                // Hiển thị cảnh báo khi tên rỗng
                $_SESSION['error_edit_course']['name'] = 'Vui lòng nhập tên khóa học.';
                header("Location:index.php?c=courses&m=edit&id=$id");
                exit();
            }

            if (empty($department_id)) {
                // Hiển thị cảnh báo khi department rỗng
                $_SESSION['error_edit_course']['department_id'] = 'Vui lòng chọn phòng ban.';
                header("Location:index.php?c=courses&m=edit&id=$id");
                exit();
            }
...
function handleAdd()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        //     $_SESSION['error_add_courses']['name'] = 'Name should contain only alphabetic characters';
        } else {
